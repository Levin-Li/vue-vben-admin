...
默认 API 前缀为 `/com.levin.oak.base/V1/api`，默认 CRUD 父级路由为 `/clob/V1/index`，子页面路由为 `/clob/V1/<Resource>`。真实菜单应以后端当前用户授权菜单为准，前端根据菜单路径匹配模块注册页面。上传页面路由时，模块 ID 必须使用后端 Java 根 POM 约定的模块包名 `com.levin.oak.base`，不能由前端自定义。
