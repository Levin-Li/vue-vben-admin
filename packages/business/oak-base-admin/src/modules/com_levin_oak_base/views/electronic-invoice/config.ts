...
export const electronicInvoicePageCrudConfig: CrudPageConfig =
  withModuleCrudConfig({
    ...
    fields: [
      ...
      { key: 'bizObjId', label: '业务对象ID', width: 160 },
      {
        key: 'title',
        label: '订单标题',
        required: true,
      ...
      { key: 'buyerName', label: '购方名称', table: true, width: 180 },
      { key: 'buyerTaxpayerId', label: '购方税号', width: 180 },
      { key: 'buyerEmail', label: '购方邮箱', width: 200 },
      { key: 'buyerMobile', label: '购方手机号', width: 150 },
      {
      ...
      { key: 'partnerSnapshot', label: '销方快照', type: 'json', form: false },
      {
        key: 'detailInfo',
        label: '商品与交付信息',
        help: '填写商品明细、税收分类编码、税率、开票人和交付邮箱等供应商所需字段；不同供应商的正式字段以官方文档为准。',
        type: 'json',
        table: false,
      },
      {
