...
import { Button, Form, InputNumber, Popover, Radio, Switch, Tooltip } from 'ant-design-vue';
...
<template>
  ...
  <section class="contents">
    <Form layout="inline" :style="{ display: 'contents' }">
      ...
      <Tooltip title="新增前端虚拟列。其值由当前行的值展示脚本计算，不对应后端实体字段。">
        <Button @click="emit('addVirtualField')">+ 添加虚拟字段</Button>
      </Tooltip>
      <Tooltip title="开启后按当前分页实际行数完整展示，不补空白行、不加载其他页；取消表格内纵向滚动，仅本页内容区域纵向滚动，保留分页和横向滚动。">
              ...
              (value) =>
                updateConfig({
                  defaultOverflowStrategy:
                    value === 'ellipsis' || value === 'wrap' ? value : undefined,
                })
            "
          />
        </Form.Item>
