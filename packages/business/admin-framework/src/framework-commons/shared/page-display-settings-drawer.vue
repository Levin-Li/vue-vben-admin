...
async function loadScopeOptions() {
  ...
  try {
    const [tenants, sites, userTypes, userCategories, orgCategories, orgTypes] = await Promise.all([
      ...
      fetchEnumOptions('com.levin.oak.base.entities.User$Category', OAK_BASE_API_MODULE),
      fetchDictOptions(
        'com.levin.oak.base.entities.Org.category',
        OAK_BASE_API_MODULE,
      ),
      fetchEnumOptions('com.levin.oak.base.entities.Org$Type', OAK_BASE_API_MODULE),
    ]);
  ...
  >
    <div class="border-border mb-4 grid grid-cols-4 gap-3 rounded border p-3">
      ...
      <Select
        ...
        show-search
      />
      <Select v-model:value="scope.userCategory" :options="userCategoryScopeOptions" placeholder="未指定（匹配任意）" allow-clear show-search />
      <Select
      ...
      >
        ...
        >
          ...
          <template v-else>
            ...
            >
            <Tooltip title="只有当前用户拥有任一选中角色时，才会看到该字段。"
              ><span>可见角色</span></Tooltip
            >
            <Tooltip title="关闭后字段不会在当前表单展示。"
              ><span>是否展示</span></Tooltip
            >
              ...
              <template v-else>
                ...
                <div class="flex gap-2">
                  <Tooltip title="编写脚本决定字段是否展示。"
                    ><Button size="small" @click="editVisibilityScript(row)"
                      >显示脚本</Button
                    ></Tooltip
                  ><Select
