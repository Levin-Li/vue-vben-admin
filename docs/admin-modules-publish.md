...
3. **阶段一：本地全量预检。** 全量构建每个包，并验证 `dist`、本地 tarball、导出、依赖协议、模块开发规范、每个包的独立安装、framework 的稳定公开入口 Vite 构建，以及 `bootstrap-app` 的生产构建。应用构建在本地工作区中使用刚完成构建的全部内部包与本地 Vite 工具链，覆盖 Oak、完整页面、Vue SFC 与运行时依赖图。临时消费者必须显式以 `file:` 引用本批每一个本地 tarball；`pnpm.overrides` 仅可在这些临时目录中将间接内部依赖固定到同一批 `file:` 制品，且不得进入发布构件。不得从 Nexus 下载本批内部构件，也不得依赖 workspace 链接、pnpm peer 提升或历史缓存。构件相关文档必须同步进入包内 `docs/` 与 `docs/project-reference/` 并通过逐文件清单校验。
