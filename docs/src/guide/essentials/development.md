...
- `@levin/bootstrap-app` 或最终业务 App：最终应用包，只负责选择模块、注入项目配置、提供页面覆盖层和最终构建产物。
...
bootstrap-app / final-system
  选择 enabled modules，按需覆盖页面，统一构建最终可部署应用
```

本仓中 `@levin/bootstrap-app` 是对外启动和打包入口。为了保持稳定和低复杂度，它当前复用 `@vben/bootstrap-app` 作为宿主实现，开发命令、构建命令都应从 `bootstrap-app` 进入：
...
```text
apps/bootstrap-app/src/pages/_core/authentication/login.vue
apps/bootstrap-app/src/pages/_core/profile/index.vue
apps/bootstrap-app/src/pages/system/shared/controller-crud-page.vue
apps/bootstrap-app/src/pages/system/com_levin_oak_base/role/index.vue
```
...
- 测试/生产/交付：必须使用 NPM 私服统一版本号，例如 `@levin/oak-base-admin@5.6.6`。
...
以`@vben/bootstrap-app`添加测试环境`test`为例：

- `apps\bootstrap-app\package.json`
...
"scripts": {
    ...
    "build:analyze": "turbo build:analyze",
    "build:bootstrap-app": "pnpm run build --filter=@vben/bootstrap-app",
    "build-test:antd": "pnpm run build --filter=@vben/bootstrap-app build:test",
...
"tasks": {
    ...
    "build-test:antd": {
      "dependsOn": ["@vben/bootstrap-app#build:test"],
      "outputs": ["dist/**"]
    },

    "@vben/bootstrap-app#build:test": {
      "dependsOn": ["^build"],
      "outputs": ["dist/**"]
    },
