...
本文档说明业务模块如何使用框架提供的前端公共基础设施，当前包括事件总线、顶部栏扩展区、用户下拉菜单扩展区和可拖拽浮动面板。全局组织与用户选择器的服务端优先级、Hook 契约和交互规则见 `../packages/business/admin-framework/docs/global-user-org-selector-hooks.md`。
