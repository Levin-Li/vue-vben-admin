...
import { userService } from '#/api/com_levin_oak_base/user';
...
async function loadMfaInfo(silent = false) {
  try {
    ...
    mfaInfo.value = (await userService.viewMyMfaQrCode()) as UserMfaInfo;
    if (!silent) {
...
function resetMfaSecretKey() {
  ...
  Modal.confirm({
    ...
    async onOk() {
      ...
      try {
        await userService.resetMfaSecretKey({
          forQrCode: true,
...
async function submitPhone() {
  if (!phoneForm.newTelephone || !phoneForm.newTelephoneVerifyCode) {
    message.warning(
      isTelephoneBound.value ? '请完整填写手机号换绑信息' : '请完整填写手机号绑定信息',
    );
    return;
  }
  ...
  try {
    ...
    message.success(isTelephoneBound.value ? '手机号换绑成功' : '手机号绑定成功');
    resetPhoneForm();
