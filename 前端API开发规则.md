...
18. 原型阶段，前端 API 方法的请求参数和返回参数可以暂用 `payload?: any` 等宽松类型，以先明确控制器归属、请求路径、请求方法、权限注解和模块前缀。页面进入稳定维护、接口依赖 DTO/JsonSchema 校验、涉及上传或权限数据，或后端 DTO 边界已稳定时，API 方法必须收紧为可表达请求和响应结构的具体类型；不得把宽松类型作为长期默认。
...
资源权限列表只使用 API 显式权限注解生成的表达式。接口 URL 是请求地址，不得作为权限备选项追加。缺少权限声明、仅声明登录或角色约束、没有资源权限字段时，资源权限列表为空；不得根据页面标题、方法名或默认领域补造权限。此规则不替代后端鉴权，也不得自动改写已保存的历史权限配置。
